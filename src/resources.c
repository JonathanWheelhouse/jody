...
void LoadGameData(void)
{
    ...
    tmp = SDL_LoadBMP(file_name("fighter.bmp"));
    if (tmp == NULL) {
    ...
    tmp = SDL_LoadBMP(file_name("back-stars.bmp"));
    if (tmp == NULL) {
    ...
    tmp = SDL_LoadBMP(file_name("front-stars.bmp"));
    if (tmp == NULL) {
...
static char * file_name(char *name)
{
	size_t dir_len = strlen(IMG_DIR);
	size_t name_len = strlen(name);
    char *all = malloc(dir_len + name_len + 2);
	char *start = all;
    all = mempcpy(all, IMG_DIR, dir_len);
    all = mempcpy(all, "/", 1);
    all = mempcpy(all, name, name_len);
	*all = '\0';
	return start;
}
